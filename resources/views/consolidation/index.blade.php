        ...
        <div class="flex flex-wrap gap-6">
            @foreach($consolidatedPRs as $cpr)
                <div class="w-full lg:w-1/2 bg-white p-6 rounded shadow">
                    <h3 class="text-xl font-bold mb-4 text-gray-800">CPR Number: {{ $cpr->cpr_number }}</h3>
                    <ul class="space-y-2">
                        @foreach ($cpr->purchaseRequests as $pr)
                            <li class="p-4 bg-gray-100 rounded">
                                ...
                                <p><strong>Order Date:</strong> {{ $pr->order_date }}</p>
                                <p><strong>Funding:</strong> {{ $pr->funding ?? 'N/A' }}</p>
                            </li>
                        @endforeach
                    </ul>
                </div>
